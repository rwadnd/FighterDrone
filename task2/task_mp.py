...
# New worker function: runs in separate process, performs model inference + per-track DB updates and annotation
def inference_worker(in_q: Queue, out_q: Queue, stop_event: Event, model_path: str, tracker_cfg: str, device):
    # Local per-worker track DB
    track_db = {}  # track_id -> TrackState

    try:
        model = YOLO(model_path)
    except Exception as e:
        # If loading fails, put an error frame (or print) and exit
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            # Send back annotated frame and outputs list
            out_q.put((frame_id, annotated, outputs))
        except Exception as e:
            # If inference fails for a frame, attempt to continue
            print("Worker: inference error:", e)
            continue

# Modified detect() to use multiprocessing worker
def detect():
    ...
    model_path = "lastv6.pt"
    device = 0 if torch.cuda.is_available() else None

    # Queues and worker setup
    ctx = mp.get_context('spawn')  # safe on Windows
    in_q = ctx.Queue(maxsize=2)    # small queue to avoid piling up old frames
    out_q = ctx.Queue(maxsize=4)
    stop_event = ctx.Event()

    worker = ctx.Process(target=inference_worker, args=(in_q, out_q, stop_event, model_path, TRACKER_CFG, device), daemon=True)
    worker.start()
    ...
    try:
        while True:
            ...
            # Try to enqueue latest frame; if queue full drop oldest to keep latency low
            try:
                if in_q.full():
                    try:
                        ...
                        pass
                in_q.put_nowait((frame_id, frame))
            except Exception:
                # if still failing, skip putting
                pass

            # Try to get result from worker (non-blocking with small timeout)
            got = False
            try:
                fid, annotated, outputs = out_q.get(timeout=0.5)
                got = True
            except Exception:
                # no processed frame ready
                annotated = frame.copy()
                outputs = []
            # Update FPS only when we received a processed frame (to reflect pipeline)
...
if __name__ == "__main__":
    # On Windows, ensure 'spawn' start method for multiprocessing
    try:
